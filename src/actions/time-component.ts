import { action, KeyDownEvent, SingletonAction, WillAppearEvent, WillDisappearEvent, DidReceiveSettingsEvent, SendToPluginEvent, Action } from "@elgato/streamdeck";
...
export class TimeComponent extends SingletonAction<TimeComponentSettings> {
	/**
	 * Map of all active time component actions and their settings.
	 */
	private static activeActions = new Map<string, { 
		action: Action<TimeComponentSettings>, 
		settings: TimeComponentSettings 
	}>();
	
	/**
	 * Global timer shared across all active time component actions.
	 */
	private static globalTimer: NodeJS.Timeout | null = null;

	/**
	 * Called when settings are updated from the property inspector.
	 * @param ev - The settings event containing updated settings
	 */
	public override async onDidReceiveSettings(ev: DidReceiveSettingsEvent<TimeComponentSettings>): Promise<void> {
	 ...
	 * @param ev - The key down event containing action and settings data
	 */
	public override async onKeyDown(ev: KeyDownEvent<TimeComponentSettings>): Promise<void> {
		await this.updateDisplay(ev.action, ev.payload.settings);
	}

	/**
	 * Called when the property inspector sends data to the plugin.
	 * @param ev - The send to plugin event containing payload data
	 */
	public override async onSendToPlugin(ev: SendToPluginEvent<TimeComponentSettings, TimeComponentSettings>): Promise<void> {
	 ...
	 * @param ev - The will appear event containing action and settings data
	 */
	public override async onWillAppear(ev: WillAppearEvent<TimeComponentSettings>): Promise<void> {
	 ...
	 * @param ev - The will disappear event containing action data
	 */
	public override onWillDisappear(ev: WillDisappearEvent<TimeComponentSettings>): void {
	 ...
	 * @param text - The text to render on the canvas
	 * @param settings - The time component settings for styling
	 ...
	 * @param settings - The time component settings containing base font size
	 ...
	 * @param date - The date object to extract time components from
	 * @param component - The specific time component to extract
	 * @param format24Hour - Whether to use 24-hour format for hours
	 * @param blinkColons - Whether colons should blink based on current time
	 * @returns The formatted time component as a string
	 */
	 ...
	 * @param action - The StreamDeck action to update
	 * @param settings - The settings for the time component
	 */
